    ...
    <div className="page-enter max-w-lg mx-auto px-4 py-8">
      ...
      <div className="rounded-2xl overflow-hidden shadow-apple-md mb-6">
        <div className="px-5 py-4" style={{ backgroundColor: team.color }}>
          <h2 className="text-lg font-bold" style={{ color: team.textColor }}>Checkout Summary</h2>
          <p className="text-sm opacity-80 mt-0.5" style={{ color: team.textColor }}>Review before confirming</p>
        </div>
        <div className="bg-white px-5 py-4 grid grid-cols-2 gap-4">
          <div>
            <p className="text-xs text-apple-tertiary mb-0.5">Person</p>
            <p className="text-sm font-semibold text-apple-dark">{personName}</p>
          </div>
          <div>
            <p className="text-xs text-apple-tertiary mb-0.5">Team</p>
            <TeamBadge team={team} />
          </div>
          <div>
            <p className="text-xs text-apple-tertiary mb-0.5">Today</p>
            <p className="text-sm font-semibold text-apple-dark">{formatDate(new Date())}</p>
          </div>
          <div>
            <p className="text-xs text-apple-tertiary mb-0.5">Return by</p>
        ...
        {cart.map(({ item, quantity }) => (
          <div key={item.id} className="px-4 py-3 flex items-center border-b border-apple-gray last:border-0">
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-apple-tertiary mt-0.5">{item.storageLocation || 'No location'}</p>
            </div>
      ...
      >
        {loading ? 'Processing…' : 'Confirm & Download Receipt'}
      </button>
    </div>
  )
}
