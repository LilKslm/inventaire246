  ...
  if (loading) {
    return (
      <div className="flex-1 flex items-center justify-center py-16">
        <div className="text-apple-tertiary text-sm">Chargement de l'inventaire…</div>
      </div>
    )
  }
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0">
      ...
      <div className="flex-1 overflow-y-auto px-4 space-y-2 pb-4">
        {filtered.length === 0 ? (
          <EmptyState icon="📦" title="Aucun article trouvé" subtitle="Essayez d'ajuster votre recherche ou filtre" />
        ) : (
          filtered.map(item => (
            <ItemRow
              key={item.id}
